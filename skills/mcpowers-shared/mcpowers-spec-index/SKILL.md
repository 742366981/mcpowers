...
| **爬虫（项目骨架）** | `爬虫规范.md` |
| **爬虫接口分析 / 逆向 / 加密参数还原** | `爬虫分析规范.md`（v2.12.0：§1.1 最终交付形态与 RPC 逆向方式分离 + §9.4 报文生命周期/真实可用性/有界并发；v2.9.5 接管 SOP §2.5.1-2.5.4、弹窗 §2.7、协作模式 §3.0、置信度 §3.4.5；v2.11.1 §2.5.5.0 安装、§2.5.5.1 daemon、§3.0.5 adapter 失败判定与结果合并） |
...
| **任何写日志 / 排查日志 / 设计日志体系**（v2.6.0）⭐ | **`日志规范.md`**（栈无关通用层：7 类日志 + JSON 字段 schema + 大内容默认截断 + 脱敏黑名单 + 级别采样 + 输出轮转；任何后端/爬虫项目必读） |
...
└── 技术规范/
    ...
    ├── Vue前端规范.md
    ├── 爬虫规范.md
    ├── 爬虫分析规范.md        # 🆕 v2.9.5 强化：§2.5.1-2.5.4 接管 SOP + §2.7 弹窗检测（8 类） + §3.0 协作模式 + §3.4.5 置信度 + 附录 D 弹窗字典库
    ├── 代码规范.md
    ├── 数据库规范.md
    ├── 缓存规范.md
    ├── Git规范.md
...
3. 加载：`代码规范.md` + **`日志规范.md`**（含 §3 字段 schema + §4 大内容截断 + §5 脱敏；按 `request_id` / `trace_id` 串联日志）
