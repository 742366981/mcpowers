...
description: |
  清理分支 / 删分支 / 帮我整理分支 / 清理无用分支 → 触发本技能。

  也覆盖口语："帮我清理一下本地分支"、"帮我清理分支"、"帮我清一下分支"、"帮我清理远程分支"、"清理分支"、"清理本地分支"、"清理一下本地分支"、"清一下分支"、"清一下本地分支"、"分支清理"、"删掉已经合并的分支"、"删掉合并过的分支"、"删掉旧分支"、"删掉无用分支"、"删除分支"、"删除旧分支"、"删除无用分支"、"删除合并过的分支"、"删除已合并分支"、"删除过期分支"、"整理一下分支列表"、"整理分支列表"、"整理分支"、"整理一下分支"、"分支整理"、"分支太乱了"、"分支太多"、"分支乱了"、"好多分支了想清一下"、"好多分支"、"分支清理一下"、"远程已删的跟踪分支还在"、"远程已删的分支清理"、"清理远程跟踪分支"、"同步远程分支"、"清理过期分支"、"清理无用分支"、"清理过时分支"、"移除无用分支"、"移除过期分支"、"删除无用的本地分支"、"无用的分支删一下"、"git branch -d"、"git branch delete"、"git clean"、"branch cleanup"、"branch clean"、"branch delete"。

  中英混输："delete branch"、"remove branch"、"clean branch"、"branch cleanup"、"cleanup branches"、"delete merged branches"、"prune"、"git fetch -p"、"remote prune"。

  清理已合并分支和远程已删的跟踪分支。**默认 dry-run 模式**（只列出待删分支，不实际删除），防止误删。
---
