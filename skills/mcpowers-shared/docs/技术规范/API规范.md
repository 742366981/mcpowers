...
| 禁止行为 | 说明 |
|:---------|:-----|
| HTTP 状态码返回非200 | 应统一返回200，通过响应体 `code` 判断成功/失败 |
| 暴露服务器具体错误信息 | 全局异常统一返回"服务器内部错误" |
| 路径传递参数 | 参数应使用 query 或 body 传递 |
| 接口不使用项目英文前缀 | 必须使用 `/{前缀}/{模块}/{操作}` 格式 |
| 单资源接口使用自定义参数名 | 单资源主键必须用 `id`，不能用 `user_id` 等 |
...
| 方法 | 适用场景 |
|:-----|:---------|
| **GET** | 查询、列表、详情、导出、下载、下拉、级联下拉 |
| **POST** | 创建、更新、删除、批量删除、导入、上传 |
...
| 下拉接口 | `/dict?type={类型值}` | `GET /ec/order/dict?type=status` | 获取指定类型的下拉选项，`type` 为字典类型代码 |
| 级联下拉 | `/dict/cascader?type={类型值}` | `GET /ec/order/dict/cascader?type=region` | 获取级联结构的下拉选项，`type` 为字典类型代码 |
...
**路径**：`POST /{前缀}/{模块}/update-status`

| 参数 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| id | int | 是 | 记录ID |
| status | int | 是 | 状态值（如 0=禁用，1=启用） |
...
> 详见 `导入导出规范.md` 第13章（表头样式、批注、数据列样式、CSV 要求、文件命名）
...
| 文档 | 位置 |
|:-----|:-----|
| 导入导出规范（含字段对照、模板设计、流程、样式、异步任务、判重策略、前端集成） | `导入导出规范.md` |
