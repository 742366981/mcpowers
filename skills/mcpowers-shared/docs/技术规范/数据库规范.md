---
title: 数据库规范
type: tech-spec
applies_to: [涉及DB]
priority: required
version: 1.0
last_updated: 2026-07-08
stability: stable
last_breaking_change: v1.0
---

# 数据库规范

本文档定义数据库操作的通用规范。

---

## 1. 适用范围

| 数据库类型 | 状态 | 说明 |
|:-----------|:-----|:-----|
| MySQL | ✅ 已支持 | 使用 SQLAlchemy ORM |
| PostgreSQL | 🔜 规划中 | 预计 v2.0 支持 |
| MongoDB | 🔜 规划中 | 预计 v2.0 支持 |
| Redis | ✅ 已支持 | 见 `缓存规范.md` |

---

## 2. ORM框架（MySQL/SQLAlchemy）

### 2.1 SQLAlchemy（MySQL/PostgreSQL）

使用 SQLAlchemy 作为 ORM 框架，统一数据库操作接口。

### 2.2 BaseModel 基类（强制）

所有数据模型必须继承 BaseModel：

```python
# db/mysql/helpers.py

from flask_sqlalchemy import SQLAlchemy
from datetime import datetime
from decimal import Decimal

db = SQLAlchemy()


class BaseModel(db.Model):
    __abstract__ = True

    def save(self):
        if self.id is None:
            db.session.add(self)
        db.session.commit()
        return self

    def delete(self):
        db.session.delete(self)
        db.session.commit()

    def to_dict(self):
        result = {}
        for column in self.__table__.columns:
            value = getattr(self, column.name)
            if value is None:
                result[column.name] = None
            elif isinstance(value, Decimal):
                result[column.name] = float(value)
            elif hasattr(value, 'isoformat'):
                result[column.name] = value.strftime('%Y-%m-%d %H:%M:%S')
            else:
                result[column.name] = value
        return result
```

---

## 3. 模型定义（MySQL/SQLAlchemy）

### 3.1 模型示例

```python
# db/mysql/models/user.py
# 用户数据模型示例（参考性最强）

from datetime import datetime
from db.mysql.helpers import db, BaseModel  # BaseModel 详见 §2.2


class User(BaseModel):
    __tablename__ = 'user'
    __table_args__ = (
        db.Comment('用户表'),
    )

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    username = db.Column(db.String(64), unique=True, index=True, comment='用户名')
    password = db.Column(db.String(128), comment='密码')
    phone = db.Column(db.String(32), comment='手机号')
    role_id = db.Column(db.Integer, index=True, comment='角色id')
    status = db.Column(db.Integer, default=1, comment='状态 0禁用 1启用')
    login_count = db.Column(db.Integer, default=0, comment='登录次数')
    last_login_time = db.Column(db.DateTime, comment='最后登录时间')
    create_user = db.Column(db.Integer, comment='创建人id')
    update_user = db.Column(db.Integer, comment='更新人id')
    create_time = db.Column(db.DateTime, default=datetime.now, comment='创建时间')
    update_time = db.Column(db.DateTime, onupdate=datetime.now, comment='更新时间')
```

---

## 4. 字段注释规范（MySQL/SQLAlchemy）

### 4.1 注释格式

| 场景 | 格式 | 示例 |
|:-----|:-----|:-----|
| 普通字段 | 中文含义 | `comment='用户名'` |
| 状态字段 | 中文含义+枚举值 | `comment='状态 0禁用 1启用'` |
| 外键字段 | 中文含义+id | `comment='角色id'` |
| 是/否字段 | 中文含义+是/否 | `comment='是否启用'` |
| 时间字段 | 中文含义 | `comment='创建时间'` |

### 4.2 字段命名与注释原则（强制）

> ⚠️ **强制要求：字段名和注释必须与需求文档保持严格一致**
>
> - **字段名**：必须与需求文档中的字段名称/含义完全一致，包括中英文含义
> - **注释**：必须与需求文档中的字段含义描述保持一致
> - **禁止自行命名**：不得自行创造、简化或扩展需求文档中未定义的字段名

| 要求 | 说明 |
|:-----|:-----|
| 名称一致 | 字段名必须与需求文档完全一致 |
| 含义一致 | 注释描述必须与需求文档中的字段含义一致 |
| 枚举值一致 | 状态字段的枚举值必须与需求文档一致 |

### 4.3 ID字段命名规范（强制）

> ⚠️ **统一使用小写 `id`**，数据库注释、API参数、代码变量保持一致

| 字段类型 | 注释格式 | 示例 |
|:---------|:---------|:-----|
| 主键 | 中文含义+id | `用户id`、`角色id`、`权限id` |
| 外键 | 中文含义+id | `角色id`、`权限id`、`创建人id`、`更新人id` |

**正确示例：**
```python
id = db.Column(db.Integer, primary_key=True, comment='用户id')
role_id = db.Column(db.Integer, comment='角色id')
permission_id = db.Column(db.Integer, comment='权限id')
create_user = db.Column(db.Integer, comment='创建人id')
update_user = db.Column(db.Integer, comment='更新人id')
```

**错误示例：**
```python
# ❌ 大小写不一致
id = db.Column(db.Integer, primary_key=True, comment='用户ID')
role_id = db.Column(db.Integer, comment='角色ID')
```

### 4.4 外键规范（强制）

**禁止使用数据库外键约束**，通过业务逻辑维护数据一致性。

```python
# 外键字段使用普通INT + 索引
role_id = db.Column(db.Integer, index=True)
```

---

## 5. 表名规范（MySQL）

### 5.1 命名格式

**表名格式：`{项目前缀}_{模块前缀}_{实体名}`**

| 组成部分 | 说明 | 示例 |
|:---------|:-----|:-----|
| 项目前缀 | 标识项目归属，建议2-6字符 | `ec`（电商）、`cbjsq`（成本计算器） |
| 模块前缀 | 标识业务模块 | `user`、`order`、`product` |
| 实体名 | 具体表名 | `info`、`detail`、`config` |

**命名规则：**
- 使用**下划线**分隔
- 全部小写
- 避免缩写，使用完整单词
- 表名体现业务含义

**示例：**
```python
# 电商项目
class User(db.Model):
    __tablename__ = 'ec_user'

class Order(db.Model):
    __tablename__ = 'ec_order_product'

# 成本计算器
class ExchangeRate(db.Model):
    __tablename__ = 'cbjsq_exchange_rate'
```

### 5.2 前缀选择建议

| 项目类型 | 建议前缀 | 说明 |
|:---------|:--------|:-----|
| 内部项目 | 2-4字符项目缩写 | 如 `sys`、`oa`、`crm` |
| 乙方外包 | 6字符防冲突前缀 | 如 `proj_{甲方简写}` |
| 多项目共享库 | 8字符唯一前缀 | 如 `ml_{项目名}` |
| 微服务 | 服务名缩写 | 如 `user_svc`、`pay_svc` |

### 5.3 表名注释规范（强制）

> ⚠️ **强制要求：表名注释必须与需求文档中的表名含义完全一致**

**命名与注释对应关系**：

| 要求 | 说明 |
|:-----|:-----|
| 表名注释来源 | 必须来自需求文档中该表的"表名含义"或"说明"列 |
| 命名一致性 | 表名与注释含义必须严格对应 |
| 禁止自行命名 | 不得自行创造、简化需求文档中未定义的表名含义 |

**SQLAlchemy 实现**：

```python
# ✅ 正确：使用 __table_args__ 定义表名注释
class User(db.Model):
    __tablename__ = 'ec_user'
    __table_args__ = (
        db.Comment('用户表'),  # 表名注释，来自需求文档
    )
    id = db.Column(db.Integer, primary_key=True, comment='用户id')
    username = db.Column(db.String(64), comment='用户名')

class Order(db.Model):
    __tablename__ = 'ec_order'
    __table_args__ = (
        db.Comment('订单表'),  # 表名注释，来自需求文档
    )
    id = db.Column(db.Integer, primary_key=True, comment='订单id')
    order_no = db.Column(db.String(32), comment='订单号')
```

**PostgreSQL 实现**（使用 alembic 或 manual DDL）：

```python
# PostgreSQL 表名注释需使用 alembic 或原生 DDL
from sqlalchemy import Table, Column, Integer, String, Text, MetaData

metadata = MetaData()
user_table = Table(
    'pg_user',
    metadata,
    Column('id', Integer, primary_key=True, comment='用户id'),
    Column('username', String(64), comment='用户名'),
    comment='用户表'  # PostgreSQL 9.5+ 支持
)
```

**检查清单**：

- [ ] 表名注释是否来自需求文档？
- [ ] 表名注释与需求文档中的含义是否完全一致？
- [ ] 是否使用 `__table_args__ = (db.Comment('表名含义'),)` ？

---

## 6. 创建人/更新人规范（MySQL/SQLAlchemy）

### 6.1 字段规范

- 使用 `create_user` / `update_user` 存储用户id，不存储用户名
- 创建时自动填充：`create_user = g.user_id`
- 更新时自动填充：`update_user = g.user_id`

### 6.2 表设计

```python
create_user = db.Column(db.Integer, comment='创建人id')
update_user = db.Column(db.Integer, comment='更新人id')
create_time = db.Column(db.DateTime, default=datetime.now, comment='创建时间')
update_time = db.Column(db.DateTime, onupdate=datetime.now, comment='更新时间')
```

---

## 7. 数据库初始化（MySQL）

> ⚠️ **强制要求：数据库初始化必须通过手动脚本执行，禁止在应用启动时自动初始化**
>
> - 后端服务启动时**不会**自动创建表或初始化数据
> - 数据库初始化通过 `db_init/init_all.py` 脚本**手动执行**
> - 每次部署或环境变更时，需手动运行初始化脚本

### 7.1 初始化脚本

```python
# db_init/init_all.py
# 数据库初始化脚本（手动执行，禁止应用启动时自动调用）

import sys
from apps import create_app
from db.mysql.helpers import db
from db.mysql.models import *  # 导入所有模型以便 create_all() 发现


def init_default_data():
    """初始化默认数据（角色、管理员、字典等）"""
    # 项目自行实现：插入默认角色、超级管理员账号、基础字典等
    pass


def init_database():
    """创建所有表 + 初始化数据"""
    app = create_app()
    with app.app_context():
        # 1. 创建所有表（基于所有继承 BaseModel 的模型）
        db.create_all()
        print('✅ 所有表已创建')

        # 2. 初始化数据
        init_default_data()
        print('✅ 默认数据已初始化')


if __name__ == '__main__':
    # 用法：python db_init/init_all.py
    # ⚠️ 注意：必须在项目根目录运行，且 ENV_TYPE 环境已配置
    print(f'初始化数据库 (ENV_TYPE={__import__("common.constants", fromlist=["ENV_TYPE"]).ENV_TYPE})...')
    init_database()
    print('🎉 初始化完成')
```

---

### 7.2 版本脚本管理（强制）

> ⚠️ **核心原则**：并不是所有数据库变更都需要新建版本目录。开发中频繁调整是常态，只有"跨版本变更"才需要新建版本目录。

#### 7.2.1 场景判断（强制）

| 场景 | 适用情况 |
|:-----|:---------|
| A. 当前版本内调整 | 未发版项目 / 当前未发布版本的小调整 / 已发版后的小调整 |
| B. 跨版本变更 | 已发版后新增需求 / 明确要发布新版本 |

**默认场景 A**（项目未发版一律选 A；已发版且影响范围大时选 B）。

**判断示例**：

```
场景 A：项目未发版、加个字段、加个字典表、修复初始化 Bug
场景 B：v1.0.0 发版后新增"订单模块"、新版本需独立数据库实例
```

#### 7.2.2 目录结构

```
db_init/
├── init_all.py                      # 初始化脚本（项目启动时）
├── v{当前版本}/
│   ├── README.md                    # 版本说明
│   └── init.sql                    # 该版本所有变更（建表+字段+数据）
└── v{新版本}/                       # 仅场景 B 需要
    ├── README.md
    └── init.sql
```

**版本号判断**：
- 未发版 → 默认 `v0.1.0` 或用户指定
- 已发版 → 取最新已发布版本号

#### 7.2.3 脚本要求

**必须包含幂等性**（无论场景 A/B）：

```sql
-- 添加字段（幂等）
ALTER TABLE users ADD COLUMN IF NOT EXISTS phone VARCHAR(20);

-- 添加数据（幂等）
INSERT IGNORE INTO roles (id, name) VALUES (1, 'admin');

-- 修改数据（幂等）
UPDATE users SET phone = '13800138000' WHERE phone IS NULL;
```

**init.sql 头部注释**：

```sql
-- =========================================
-- 版本：v1.0.0
-- 说明：累计变更（场景 A）或本版本变更（场景 B）
-- 幂等：是（重复执行会忽略）
-- 最后更新：YYYY-MM-DD
-- =========================================
```

#### 7.2.4 执行流程

**场景 A**（追加到当前 init.sql）：

```
1. 追加 SQL 到 db_init/v{当前版本}/init.sql
2. 在开发/test 环境执行该脚本
3. 验证（幂等性保证可重复执行）
```

**场景 B**（新建版本目录）：

```
1. 新建 db_init/v{新版本}/init.sql
2. 在目标环境执行该脚本
3. 晋升时确认已执行
```

#### 7.2.5 晋升检查清单

> ⚠️ **人工检查项**
>
> 环境晋升时，人工需确认：
> - [ ] 是否有新的数据库脚本？
> - [ ] 脚本是否已在当前环境执行？
> - [ ] 执行后数据是否符合预期？

---

## 8. 命名规范

### 8.1 数据库字段

| 类型 | 规则 | 示例 |
|:-----|:-----|:-----|
| 普通字段 | 下划线 | user_name |
| 外键 | xxx_id | role_id |
| 状态 | status (0禁用/1启用) | status |
| 时间 | create_time, update_time | create_time |
| 创建人/更新人 | create_user/update_user (存用户id) | create_user |

### 8.2 Python变量

| 类型 | 规则 | 示例 |
|:-----|:-----|:-----|
| 普通变量 | 下划线 | user_list |
| 常量 | 全大写 | PAGE_SIZE |
| 类名 | 大驼峰 | UserService |

### 8.3 API路径

| 类型 | 规则 | 示例 |
|:-----|:-----|:-----|
| URL | 中横线 | /user-info |
| 文件名 | 下划线 | user_info.py |

---

## 附录

### A. 相关文档

| 文档 | 位置 |
|:-----|:-----|
| API规范 | `API规范.md` |
| 缓存规范 | `缓存规范.md` |
| Flask后端规范 | `Flask后端规范.md` |
