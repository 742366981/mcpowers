...
| 规范 | 检查命令/方式 | 预期结果 |
|:-----|:-------------|:---------|
| **开发环境规范** | `python --version`、`pip list` | 版本正常 |
| **Git规范** | `git status` | 工作区状态 |
| **Flask后端规范** | 检查 `apps/` 目录结构 | 符合应用工厂模式 |
| **API规范** | 检查 `docs/API文档/` 是否存在 | 存在或按需创建 |
| **代码规范** | 检查 `SOLID/KISS/DRY/YAGNI` | 代码符合原则 |
