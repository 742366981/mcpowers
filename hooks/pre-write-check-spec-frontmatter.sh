...
# v2.27.4+：检测技术规范 frontmatter 是否缺失 / 删除 stability / last_breaking_change 字段
# 复用 pre-write-check-import.sh 的薄 wrapper 模式：
#   - 探测 python 解释器
#   - 转发 stdin JSON 到 Python 检测器
#   - exit 0 = 放行，exit 2 = 触发 confirm UI

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DETECTOR="$SCRIPT_DIR/check_spec_frontmatter.py"
