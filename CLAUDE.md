...
**代码/配置零引用铁律·智能二分判定（v4.3.0+ 用户决策 D·全栈适用）**：AI 写任何代码注释（`#` 单行 / `"""` docstring / `//` JS / `--` SQL）/ 配置文件（`.yaml` / `.yml` / `.json` / `.ini` / `.toml`）/ `.sh` 头部注释都需遵守——**22 个禁用字眼**（同 R15/R16 共享清单）+ **v4.3.0 新增 4 个口语化补充**（遵循本项目规范 / 遵循团队规范 / 遵循本仓库规范 / 按团队规范）触发**智能二分判定**：**优先级 1 边界豁免**（YAML 字段名行 / Issue 编号 / `${CLAUDE_PLUGIN_ROOT}` 占位符 / swagger 5 字段值）→ 放行；**优先级 2 外部权威**（RFC/PEP/W3C/WHATWG/WCAG/OWASP/ASVS/CWE/CVE/IEEE/ISO/IEC/ECMA/Unicode/SQL/NIST/FIPS/中国国标/POSIX + 公认作者 + 官方 URL + 行业+规范前缀）→ 放行；**优先级 3 内部规范名**（33 份规范 + 别名，共享常量 `_internal_spec_docs.txt`）→ 拦截（internal_spec）；**优先级 4 项目内代码文件路径**（`utils/xxx.py` 等）→ 拦截（internal_code）；**优先级 5 项目内 .md 文档名**（含 CLAUDE.md/README.md/AGENTS.md，**用户决策：无例外**）→ 拦截（internal_md）；**优先级 6 无前缀画蛇添足**（「按规范/根据规范/遵守规范」无外部权威前缀）→ 拦截（unbounded_spec）；**优先级 7 兜底拦截**（含禁用字眼既不外部也不内部）→ 拦截（fallback）。**路径白名单 6 类**（tests/ / fixtures/ / examples/ / templates/ / docs/历史教训/ / CHANGELOG.md）+ **docstring 降级为 WARNING**（避免整段 docstring 全 ERROR 阻塞）+ **YAML 字段名行跳过**（结构标记放行）。详见 [`代码规范.md`](skills/mcpowers-shared/docs/技术规范/代码规范.md) §11.3.1（智能二分 + 14 反例 + 6 正例权威定义）。栈级落地：共享检测器 `skills/mcpowers-shared/scripts/check_no_ref_words.py`（v4.3.0 新增，被 3 个 hook 共用）；共享常量 3 份 `_forbidden_ref_words.txt` + `_internal_spec_docs.txt` + `_external_authority.txt`（与 R15/R16 共用 `_forbidden_ref_words.txt`，避免漂移）；硬门禁 hook `pre-write-check-no-ref-words.sh`（PreToolUse Write → 智能二分 → ERROR 级 → exit 2 confirm UI）+ 软门禁 hook `post-write-check-no-ref-words.sh`（PostToolUse Write|Edit|MultiEdit → WARNING 级兜底，Edit 无 content 仅靠此处兜底）；审查门禁：`mcpowers-code-review` 增 R17 反模式条目（代码/配置含禁用引用字眼，3 优先级 + 智能二分 + 路径白名单）+ Quick-Check 段含 **8 条**扫描命令（22 字眼扫描 + docstring 降级 + 外部权威放行 + 内部规范拦截 + 代码文件拦截 + CLAUDE.md/README.md 拦截 + 无前缀拦截 + 共享常量存在性）。
