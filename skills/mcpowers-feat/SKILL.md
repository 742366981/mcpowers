---
name: mcpowers-feat
description: |
  加个功能 / 加功能 / 做个新功能 / 新增功能 / 新做一个 / 实现一下 → 触发本技能。

  也覆盖口语："我想加个新功能"、"我想做个新功能"、"我要加个功能"、"我要做个 XX"、"我要开发 XX"、"帮我做一个 XX"、"帮我开发 XX"、"帮我实现 XX"、"帮我加一个 XX"、"帮我做个 XX"、"帮我加 XX"、"新增一个 XX"、"加一个 XX"、"加一个新 XX"、"做一个 XX"、"做个 XX 功能"、"做个新 XX"、"加个 XX 功能"、"开发一个 XX"、"实现一个 XX"、"实现 XX"、"搭一个 XX 模块"、"搭一个新模块"、"新建一个 XX"、"新建 XX"、"创建 XX 功能"、"创建 XX"、"我要做 XX"、"我要实现 XX"、"开发 XX 功能"、"写一个新功能"、"写一个 XX 功能"、"加个 XX 接口"、"加个 XX 页面"、"加个接口"、"加个页面"、"加个模块"、"做个新接口"、"做个新页面"、"做个新模块"、"搞个 XX"、"弄一个 XX"、"整一个 XX"、"做一下 XX"、"加上 XX"、"给我加 XX"、"给我做个 XX"、"加进来"、"新加一个"。

  中英混输："add feature"、"add"、"implement"、"create"、"develop"、"feat"、"feature"、"new"、"add a"、"build"、"ship"。

  边界（防误触发）：**改既有功能、加字段、调流程、改逻辑** → `mcpowers-requirement-change`；**修复 bug** → `mcpowers-bugfix`。

  编排新功能开发的迷你工作流：澄清需求 → 任务拆解 → 按规范实现 → TDD 测试 → 自审 → 提交。覆盖 80% 日常开发场景。
---

# mcpowers-feat（加功能）

> 借鉴自 superpowers `writing-plans`。
> **核心**：只编排流程，不复制规范内容；规范按需 Read。

---

## 编排

本技能按顺序调用以下方法层技能 + 规范：

| 步骤 | 调用对象 | 类型 | 触发条件 | 失败时 |
|:-----|:---------|:-----|:---------|:-------|
| 1 | `mcpowers-brainstorm` | 方法 | 需求不清 / 多义 | 中断并回问用户 |
| 2 | `mcpowers-plan` | 方法 | 任务 > 3 步 | 跳过（不强制） |
| 3 | 规范组（见 mcpowers-spec-index） | 规范 | 必读 | 中断，提示加载失败 |
| 4 | `mcpowers-tdd` | 方法 | 必走 | RED 缺失则不进入下一步 |
| 5 | `mcpowers-code-review` | 方法 | 实现完成 | Critical 必须修复 |
| 6 | `mcpowers-git-commit` | 场景 | 自审通过 | 阻断提交直到修复 |

**保护路径**（PreToolUse(Write) hook 强制确认）：`mcpowers-shared/`、`mcpowers/`、`hooks/` 三个目录的写操作触发前确认。

**不允许跳过**：标注"必走"或"必读"的步骤必须执行。

---

## 触发即执行（9 步）

### 1. 澄清需求
- 需求**清楚** → 跳过，直接进入第 2 步
- 需求**不清** → 调 `mcpowers-brainstorm` 澄清
- 用户明确说"按这个 PRD 做" → 跳过澄清，直接进入第 2 步

### 2. 任务拆解
- 任务 **≤ 3 步** → 跳过拆解，直接进入第 3 步
- 任务 **> 3 步** → 调 `mcpowers-plan` 拆成 2-5 分钟可验证小任务

### 3. 加载规范
- **必须** Read `mcpowers-shared/mcpowers-spec-index/SKILL.md`
- 按查表结果加载对应规范（基线 + 栈规范 + 场景规范）
- 常见组合：
  - Flask 后端接口 → `代码规范` + `Flask后端规范` + `API规范`
  - Vue 页面 → `代码规范` + `Vue前端规范` + `设计规范`
  - 爬虫 → `代码规范` + `爬虫规范`
  - 涉及 DB → 加上 `数据库规范`
  - 涉及缓存 → 加上 `缓存规范`

### 4. 接口先写文档（强制）
- 接口 / 公共函数 / 类：先写 docstring，再写实现
- 详见 `mcpowers-shared/docs/技术规范/Flask后端规范.md` 第 13 章（如加载了）

### 5. TDD 循环
- **调 `mcpowers-tdd`**，按 RED-GREEN-REFACTOR 循环
- **铁律**：没有失败的测试，不写生产代码
- 小步快走：每步可独立运行验证

### 6. 按规范实现
- 严格遵守已加载的规范
- SOLID / KISS / DRY / YAGNI 是基线
- 改动波及多处 → Read `代码同步修改规范.md` 找全引用点

### 7. 自审
- 调 `mcpowers-code-review`，多维并行审查
- Critical 问题立即修复，Important 评估后处理

### 8. 端到端验证（强制）— 不可跳过

> **铁律**：单测全绿 ≠ 系统正常。新功能首次合入最容易在集成层（接口契约、配置加载、依赖服务、数据库迁移）出问题，必须重启服务跑一遍主链路。

**展开内容**：Read `mcpowers-shared/docs/技术规范/测试规范.md` 第 9 章「端到端自检 5 步」。

**快速版（5 步）**：

1. **重启服务**：dev/staging 环境实际启动一次（不是 `pytest` 跑过就完事）
2. **主链路 curl**：至少跑通 1 个 happy path 接口 / 核心功能点
3. **日志无 ERROR**：服务启动后 5 分钟内日志无 ERROR/WARN 异常
4. **回归点走过**：新功能依赖的上下游接口 / 数据流已验证
5. **环境就绪**：配置/数据库迁移/外部依赖（如有）都已生效

**失败处理**：任何一步失败 → 回 Step 6 修复，禁止带病进入收尾。

### 9. 收尾
- 记录重要细节 → 按需 Read `细节记录规范.md` 后追加
- 调 `mcpowers-git-commit` 提交（按 `Git规范.md`）
- 代码和文档必须同 commit

---

## 何时中断并询问用户

- 需求中存在歧义（"做一个登录"是密码登录？手机号？第三方？）
- 多个方案都不明显占优（"用 Redis 还是用数据库存 session"）
- 改动影响范围超出预期（"改一个字段牵连 5 个模块"）
- 涉及架构变更（新增服务 / 改数据流）

---

## 反模式（禁止）

- ❌ 一上来就写代码，不读规范
- ❌ 一开始就 Read 所有规范文件（爆上下文）
- ❌ 跳过 TDD 直接写实现
- ❌ 写完代码不测试就自审
- ❌ 改完不 commit / 只 commit 代码不 commit 文档
- ❌ **单测全绿就提交，跳过端到端验证**（集成层故障看不见）

---

## 完成后自检清单

- [ ] 规范已按需加载（不是全量）
- [ ] 接口/函数有完整 docstring
- [ ] 测试覆盖核心逻辑
- [ ] 自审通过（无 Critical 问题）
- [ ] **端到端自检 5 步已通过**（见 Step 8）
  - [ ] 服务已重启
  - [ ] 主链路接口 curl 通过
  - [ ] 日志无 ERROR/WARN
  - [ ] 回归点已验证
  - [ ] 环境/配置/迁移已就绪
- [ ] 重要细节已记录
- [ ] 代码和文档已同 commit
