...
**v2.27.6 补充：重复检测启发式精细化**——上述 hook 不再纯按函数名一刀切，按 4 类启发式分级：①单行透传（`return <已有函数>(...)` 一行包转发）→ gold standard 强化阻断（exit 2 + confirm UI），无论是否触发后续降级都拦下；②同命名空间跨段（`utils/a.py::format_response` vs `utils/b.py::format_response`）/ ③签名差异（参数列表归一化后不同）/ ④绑定对象不同（`def foo(self, ...)` 命中模块函数或反之）→ 视为合法重名，降级 warn（exit 0 + stderr 写 `⚠ [降级 · 合法重名·<理由>]` 提示，**不弹 confirm UI**）。详见 [`代码规范.md`](skills/mcpowers-shared/docs/技术规范/代码规范.md) §6.1.1 v2.27.6 补充段。审查门禁：`mcpowers-code-review` 增 R10「二次包装 vs 合法重名未区分」反模式条目 + 「v2.27.6+ 启发式分级 Quick-Check」（单行透传 rg 命令）。
