...
description: "清理分支 / 删分支 / 帮我整理分支 / 清理无用分支 → 触发本技能。口语：帮我清理本地/远程分支、删掉已合并/旧/无用分支、删除分支/过期分支、整理分支列表、分支太乱/太多/想清一下、远程已删的跟踪分支还在、清理远程跟踪分支/同步远程分支、移除过期分支。中英：delete branch, remove branch, clean branch, branch cleanup, prune, git fetch -p, remote prune。保留 main/master/develop；默认 dry-run 防误删。"
---
