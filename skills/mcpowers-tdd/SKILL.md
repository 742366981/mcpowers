---
name: mcpowers-tdd
description: "TDD / 测试驱动 / 先写测试 / 红绿循环 / 测试先行 → 触发本技能。口语：用 TDD 写代码/做、走测试驱动开发/走 TDD 流程、按红绿循环做、先写测试再写实现、先测试后代码、单测先行、RED-GREEN-REFACTOR/红绿重构、强制 TDD/TDD 模式、写测试/写测试用例先。中英：TDD, test-driven, test first, RED-GREEN-REFACTOR, BDD。边界：纯文档/配置/UI 文案跳 TDD；修 bug 用 TDD 但先写复现测试；纯补测试→`mcpowers-code-review`。先失败测试→最少代码→重构。"
---
...
> 💡 **测试数据命名**（v2.0.5）：fixture / mock 数据使用 `test_` / `tmp_` 前缀（详见 `测试规范.md` §7.1.1），便于后续清理。
...
> 💡 **测试数据自动清理**（v2.0.5）：单测通过后，§3.1 conftest 的 `db_session` fixture 会自动 rollback 测试期间产生的脏数据，无需手动 SQL 清理。
