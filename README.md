...
- 完整支持 5 个 Hook 事件组 / 12 个脚本（`SessionStart` + `PreToolUse(Bash/Write/Edit)` + `PostToolUse`，含 v2.27.4+ `pre-write-check-spec-frontmatter.sh` + v2.29.0+ `pre-write-check-doc-sync.sh` + **v4.3.0+ `pre-write-check-no-ref-words.sh` 硬门禁（v4.5.2+ 已扩展至 Edit/MultiEdit）+ `post-write-check-no-ref-words.sh` 软兜底 + 共享 `skills/mcpowers-shared/scripts/check_no_ref_words.py` 智能二分检测器（v4.5.2+ 兼容 `new_string` / `edits[*].new_string` 输入格式）** + **v4.5.2+ `post-write-check-swagger.sh`（PostToolUse 兜底：PreToolUse Swagger 钩子从磁盘读旧文件，Edit/MultiEdit 模式漏检——v4.5.2 新增 PostToolUse 钩子在文件已写入后跑同一 `swagger-contract-check.sh` helper，命中违规 → exit 2 → Claude Code 自动修正）** + **v4.5.2+ duplicate 函数检测器 MultiEdit 支持**（复用 no-ref-words 同模式 3 档归一化，MultiEdit 也走「同文件重名 / 跨文件单行透传」两类 block） + **v4.5.2+ spec-frontmatter MultiEdit 行为锁定**（v2.27.4 起已支持，v4.5.2 仅加 §7.8 T18 回归测试防退化）——代码/配置零引用铁律,7 优先级判定,R15 接口零引用 + R16 .md 零引用 + R17 代码/配置零引用三层铁律共用 22 字眼清单）
