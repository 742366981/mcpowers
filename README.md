...
| 3.5 | **📝 Swagger 接口契约硬门禁**（v2.31.0+ 写时硬拦；v4.0.0+ 业务接口只列 200；v4.0.1+ 接口文档零引用；v4.4.0+ description 零冗余 + 通用响应/分页 `$ref` 复用；v4.5.0+ 四铁律 + v4.5.1+ POST 强制 JSON + **v4.5.2+ Edit/MultiEdit PostToolUse 兜底**） | 写接口文件时 PreToolUse 阶段物理拦截 5 字段契约（`tags` + `summary` ≤ 30 字 + `description` ≤ 100 字 + `parameters` 含 `description`+`example` + `responses` 含 200 + 每个状态码含 `schema`+`examples`）不合规的写法，避免 commit 时一次性报 20+ 错误导致返工；**v4.0.0+ 业务接口 `responses` 只列 `200`**（业务错误走响应体 `code` 字段；4xx/5xx 由框架层抛出），`swagger-lint-helper.py check_business_api_responses` 写时拦截；**v4.0.1+ 接口文档零引用铁律**——`summary` / `description` / `parameters[].description` / `responses[].description` 等字段值禁含「参考 / 参见 / 详见 / 引用」+「according to / refer to / see also」等指向其他文档的字眼，让对接方只看接口文档就能调用，`swagger-lint-helper.py check_no_reference_words` + `export_docs.py check_no_reference_words_spec` 双层拦截；**v4.0.2+ 文档编写铁律·画蛇添足字眼场景化规则**（v4.0.1 接口零引用的通则化推广）——AI 写任何文档（不只是接口）都需遵守 22 字眼清单 + 3 问决策（输出型禁止 / 参考型允许且必要 / 历史型允许），CLAUDE.md 必读铁律段 + 6 文档场景技能 description 触发词 + `post-write-check-doc-content.sh` 软门禁 + R16 审查门禁 6 层 AI 视野自动覆盖；**v4.4.0+ 接口文档 SSOT 终态收敛**——description 字段禁用 8 类内容（HTTP 状态码 / 认证方式 / 错误码清单 / 响应结构 / 完整路径 / 通用约束 / 路径内模块名 / summary 同义重复）；通用响应/分页/认证用 `$ref` 复用 5 个全局组件（`StandardResponse` / `BizResponse` / `PageResponse` / `BizError` / `FileResponse` + `BearerAuth`），接口路径不重复 `basePath` / 蓝图前缀——`swagger-lint-helper.py check_description_redundant_content` / `check_no_path_in_description` / `check_no_repeated_schema` 3 个新检查函数 + `swagger_components.md` + `flask_swagger_config.py` 落地资产；**v4.5.0+ 四铁律 ERROR 硬门禁**（用户决策 D 续·不设 WARNING 过渡期）——`§1.G` 路径禁动态参数（`@bp.route` 模板禁含 `<xxx>` / `<int:xxx>`，资源标识走 query/body；例外 `webhook` / `oauth` / `callback`）+ `§1.H` HTTP 方法白名单（业务接口 HTTP 只允许 GET 或 POST，禁止 PUT/PATCH/DELETE/HEAD/OPTIONS）+ `§1.I` description 禁鉴权字眼（JWT / Bearer / 需登录 不在 description 重述）+ `§1.J` description 禁错误码清单（错误码统一在 `responses.examples` 或 `$ref BizError` 组件维护）——`swagger-lint-helper.py check_no_dynamic_path` / `check_allowed_methods` / `check_no_auth_in_description` / `check_no_error_codes_in_description` 4 个新检查函数（AST 解析 `@bp.route` 装饰器）；**v4.5.1+ §1.K POST 强制 JSON 铁律**（新增第 5 条）——业务接口 POST 一律 `Content-Type: application/json`，禁 form-urlencoded / multipart；豁免白名单 `/upload` / `/import` / `/attachment` / `/webhook` / `/callback` / `/notify` / `/oauth` 路径段——`swagger-lint-helper.py check_post_must_be_json` 5 个新检查函数；项目根可放 `.swagger-required-fields.yml` 自定义必填字段；未装 swagger 的项目零摩擦放行；不向用户项目注入任何文件；**v4.5.2+ PostToolUse 兜底**——PreToolUse 钩子从磁盘读 `file_path`（Write 模式新文件未落盘 / Edit/MultiEdit 模式读到旧文件），用户在 Edit 模式新增 `@bp.route` 时 PreToolUse 看到旧文件无新函数 → 静默放行（用户报告『api 规范没生效』的根因）；v4.5.2 新增 `post-write-check-swagger.sh` PostToolUse 钩子——文件已写入后从磁盘读最新内容，跑同一 `swagger-contract-check.sh` helper，命中违规 → exit 2 → Claude Code 把 stderr 喂回 Claude 自动修正 |
| 4 | **🧪 方法论复用** | TDD 强制先写测试、Brainstorm 澄清需求、Plan 任务拆解、Code Review 铁律，被场景层按需编排 |
| 5 | **🛡️ 铁律双约束** | 软约束（技能描述里的 `铁律` + `## 反模式（禁止）` ❌ 清单）+ 硬约束（Claude Code hooks 物理阻断危险命令 + v2.28.2 重复函数检测（极简：跨文件同名默认放行 + 同文件重名 / 单行透传 wrapper 两类 block；豁免 `main` / `hook_main` 入口惯例 + Python dunder + 单下划线私有名；v4.5.2+ MultiEdit 模式已支持）+ v2.27.0 Python 局部 import 拦截 + v2.27.4 stability / last_breaking_change / CHANGELOG Breaking Changes 段强制声明 + **v2.31.0 Swagger 5 字段契约硬门禁**（写时即拦，避免返工；v4.5.2+ PostToolUse 兜底 → Edit/MultiEdit 模式新增 @bp.route 时 PreToolUse 读到旧文件漏检自动修正）） |
| 6 | **🪝 5 个事件组 / 12 个 Hook 脚本**（v4.4.0+ description 零冗余 + 通用响应 `$ref` 复用三检；v4.5.0+ 接口契约四铁律 + v4.5.1+ POST 强制 JSON ERROR 硬门禁；v4.5.2+ 新增 `post-write-check-swagger.sh` PostToolUse 兜底；**v4.6.3+ 22 字眼硬门禁从 PreToolUse 迁移到 `pre-bash-guard.sh` 的 git commit 兜底**——解决 AI 每行被 confirm UI 阻塞 + AI 同义词绕开封闭集字眼表两大痛点） | SessionStart 注入铁律、PreToolUse(Bash) 阻断 `rm -rf /` **+ v4.6.3+ git commit 时扫暂存区所有变更文件检测 22 字眼（ERROR 级违规 → exit 2 阻断提交）**、PreToolUse(Write) 保护核心目录并提示接口文档同步 + 重复函数检测（v2.28.2+ 极简：跨文件同名默认放行 + 同文件重名 / 单行透传 wrapper 两类 block；**v4.6.3+ lazy 化**——仅命中同文件重名或单行透传 wrapper 候选时才跨文件扫描，普通 Edit 不再全仓 walk）+ Python 局部 import 拦截（v2.27.0+）+ 规范 frontmatter 双字段强制声明（v2.27.4+）+ **doc-sync 物理门禁（v2.29.0+：path/route/env 三类检查，替代 v2.9.0 引入的 `doc-sync-install` 技能 [已废弃]，装 mcpowers 即自动支持，**不向用户项目注入任何文件**）+ **Swagger 5 字段契约硬门禁（v2.31.0+：写接口文件即拦，避免返工）** + **文档画蛇添足字眼软门禁（v4.0.2+：写 .md 即扫 22 字眼，6 类路径白名单区分参考型/历史型文档）** + **v4.4.0+ description 字段 8 类禁用内容 + 完整路径 + 重复 schema 三检（WARNING 渐进迁移阶段）** + **v4.5.0+ 接口契约四铁律 ERROR 硬门禁（§1.G 路径禁动态参数 / §1.H HTTP 方法白名单 / §1.I description 禁鉴权 / §1.J description 禁错误码清单——不设 WARNING 过渡期）** + **v4.5.1+ §1.K POST 强制 JSON 铁律（POST 一律 application/json，禁 form-urlencoded/multipart；豁免 upload/import/attachment/webhook/callback/notify/oauth 路径段）**、PostToolUse 提醒提交 |
...
│   ├── mcpowers-min-module/            # 最小通用模块化（v2.28.0 新增，零业务自包含四件套 + 跨项目可搬运）
...
mcpowers 通过 `hooks/hooks.json` 声明 4 个 Claude Code Hook 事件组，并调用 6 个脚本，由插件系统自动注册（**无需修改 `settings.json`**），让铁律从"软提示"升级为"硬约束"。
...
| `PreToolUse (Write)` | 每次 Write 文件前 | `pre-write-confirm.sh` 保护核心资产；`pre-write-confirm-api-hint.sh` 在接口相关文件变更时**物理硬门禁 Swagger 5 字段契约（v2.31.0+：栈探测 → 字段清单加载 → 单文件 lint → exit 2 触发 confirm UI，避免返工；未装 swagger 项目零摩擦放行）** |
...
| AI 工具 | 支持状态 | 安装方式 | 说明 |
|:--------|:--------:|:---------|:-----|
| **Claude Code** | ✅ **完全支持** | `/plugin install mcpowers@mcpowers` | 路由器 + 32 个可路由技能 + 32 技术规范 + 5 个 Hook 事件组 / 12 个脚本全功能 + v4.3.0+ 代码/配置零引用智能二分 + v4.5.2+ Swagger PostToolUse 兜底 |
...
- 完整支持 5 个 Hook 事件组 / 12 个脚本（`SessionStart` + `PreToolUse(Bash/Write/Edit)` + `PostToolUse`，含 v2.27.4+ `pre-write-check-spec-frontmatter.sh` + v2.29.0+ `pre-write-check-doc-sync.sh` + **v4.6.3+ 22 字眼硬门禁迁移到 `pre-bash-guard.sh` 的 git commit 兜底段**（从 PreToolUse Write/Edit/MultiEdit 硬门禁迁移——解决每行被 confirm UI 阻塞 + AI 同义词绕开两大痛点；`post-write-check-no-ref-words.sh` 软兜底保留开发期 stderr 提示）+ **v4.5.2+ `post-write-check-swagger.sh`（PostToolUse 兜底：PreToolUse Swagger 钩子从磁盘读旧文件，Edit/MultiEdit 模式漏检——v4.5.2 新增 PostToolUse 钩子在文件已写入后跑同一 `swagger-contract-check.sh` helper，命中违规 → exit 2 → Claude Code 自动修正）** + **v4.5.2+ duplicate 函数检测器 MultiEdit 支持**（v4.6.3+ lazy 化：仅命中同文件重名或单行透传 wrapper 候选时才跨文件扫描，普通 Edit 不再全仓 walk） + **v4.5.2+ spec-frontmatter MultiEdit 行为锁定**（v2.27.4 起已支持，v4.5.2 仅加 §7.8 T18 回归测试防退化）——共享 `skills/mcpowers-shared/scripts/check_no_ref_words.py` 智能二分检测器（v4.5.2+ 兼容 `new_string` / `edits[*].new_string` 输入格式；v4.6.3+ 改由 `pre-bash-guard.sh` git commit 兜底段调用，7 优先级判定；R15 接口零引用 + R16 .md 零引用 + R17 代码/配置零引用三层铁律共用 22 字眼清单）
...
| 5 | 跑 `bash tests/plugin-verify.sh` 确认断言阈值(≥28)仍通过——plugin-verify 内部对规范总数设下限,新规范新增到 ≥29 时需要更新该阈值 |  |
...
3. `CLAUDE.md` 和 `README.md` 同步更新 Hook 事件组/脚本清单及维护说明（v4.3.0+ 累计 11 个脚本 / 5 个事件组；v4.5.0+ 接口契约四铁律 ERROR 硬门禁加入清单；v4.5.2+ 累计 12 个脚本，新增 `post-write-check-swagger.sh`）
