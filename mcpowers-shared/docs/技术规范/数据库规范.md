...
> ⚠️ **适用场景**：版本发布后新增需求涉及数据库变更（新增表、字段、数据迁移等）
...
```
{项目缩写}_{版本号}_{序号}_{简短描述}.sql
```

示例：
```
sso_v1.0.0_001_create_user_table.sql
sso_v1.1.0_001_add_phone_field.sql
```
...
CREATE TABLE IF NOT EXISTS schema_version (
    ...
    remark VARCHAR(500),
    UNIQUE KEY uk_version_script (version, script_name)
);
```
...
```
1. 开发时：如有数据库变更，在 db_init/v{版本号}/ 下创建脚本

2. dev → test 晋升前：
   └→ 检查 db_init/v{版本号}/ 下是否有新增脚本
   └→ 手动执行脚本（需有数据库权限）
   └→ 验证通过后晋升代码
