...
| `hooks/` | Claude Code hooks 资产（5 个事件组 / 12 个脚本 + `hooks.json`（v4.5.2+ 含 `post-write-check-swagger.sh` Swagger 接口契约兜底）；v2.28.2+ 含 `pre-write-check-duplicate.sh` 重复函数检测（v4.6.3+ lazy 化：仅命中同文件重名或单行透传 wrapper 候选时才跨文件扫描，普通 Edit 不再全仓 walk；极简：跨文件同名默认放行 + 同文件重名 + 单行透传 wrapper 两类 block；豁免 `main` / `hook_main` 入口惯例 + Python dunder 协议方法 + 单下划线私有名）；v2.27.0+ 含 `pre-write-check-import.sh` Python 局部 import 拦截；v2.27.4+ 含 `pre-write-check-spec-frontmatter.sh` 规范 frontmatter 字段强制声明；**v2.29.0+ 含 `pre-write-check-doc-sync.sh` doc-sync 物理门禁（path/route/env 三类检查）——替代 v2.9.0 引入的 `doc-sync-install` 技能 [已废弃]，不向用户项目注入任何文件，装 mcpowers 即自动支持**；**v4.6.3+ 22 字眼门禁迁移：从 PreToolUse Write/Edit/MultiEdit 硬门禁迁移到 `pre-bash-guard.sh` 的 git commit 兜底扫描（解决 AI 写内容每行被 confirm UI 阻塞 + AI 同义词绕过封闭集字眼表两大用户痛点）；`post-write-check-no-ref-words.sh` 软门禁保留（开发期 stderr 提示不阻塞）；共享 `scripts/check_no_ref_words.py` 检测器 + 3 份共享常量（`_forbidden_ref_words.txt` + `_internal_spec_docs.txt` + `_external_authority.txt`）——智能二分判定代码/配置零引用字眼，覆盖 R15（接口零引用）+ R16（.md 零引用）+ R17（代码/配置零引用）三层铁律**） |
...
**写 Swagger 接口必须按 5 字段契约（v2.31.0+ 全栈适用铁律；v4.0.0+ 业务接口只列 200；v4.0.1+ 接口文档零引用；v4.2.0+ 表格排版防护 + XSS 阻断）**：任何使用 Swagger / OpenAPI 工具栈的项目（flasgger / apispec / fastapi / springdoc / openapi-typescript 等）写接口文件（views.py / /views/ / router.{py,js,ts} 或 /router/ / /controllers/ / /api/ / /routes/ / /handlers/ / /endpoints/ / /urls.py / /resources/ / /blueprints/）时，**PreToolUse 阶段物理门禁**——`tags` + `summary`（≤ 30 字，句末无标点）+ `description`（≤ 100 字简短功能说明）+ `parameters`（每个含 `description` + `example`）+ `responses`（含 200，每个状态码含 `schema` + `examples`）任一缺失视为不完整接口，触发 Claude Code confirm UI（exit 2）。**v4.0.0+ 业务接口 `responses` 只列 `200`**——业务成功/失败由响应体 `code` 字段判断（`code: 0` = 成功；非 0 = 业务失败），4xx/5xx 仅由框架层（Flask abort / Webargs / Flask-JWT-Extended 中间件）抛出，不由业务接口在 docstring 声明；认证接口（路径含 `login`/`logout`/`refresh`/`verify`/`register`/`password`）+ 流式接口（路径含 `download`/`export`/`stream`/`upload`/`file`/`attachment`）为例外，可保留 401/416。**v4.0.1+ API 文档零引用铁律**——接口文档（docstring → spec → md 全链路）应聚焦"怎么对接调用"，`summary` / `description` / `parameters[].description` / `responses[].description` 等用户可见字段值**禁用字眼**：「参考 / 参见 / 详见 / 引用 / 参照 / 引自」+「根据规范 / 按照规范 / 按规范要求 / 遵守规范 / 按规范」+「according to / refer to / referring to / as described in / as specified in / see also」——这些字眼会让对接方以为还要再去查其他文档才能用。**v4.2.0+ 表格排版防护 + XSS 阻断**——`export_docs.py` 导出 markdown 时,所有 7 个表格生成点（含顶层 description）走 `_md_cell_safe()` 4 步走：① 规范化（None / dict / list → str）→ ② 不可见字符清理（NBSP / EM SPACE → 空格；ZWSP / BOM → 删）→ ③ Markdown 转义（`\` → `\\`、`|` → `\|`、换行 → `<br>`）→ ④ 危险结构防御（HTML 标签剥离白名单 `<br>`、`|---` 等分隔行冒充 → `—`、代码块围栏 → 单 `` ` ``、列表 / 标题前缀 → 空格）；`_scan_xss_risk()` 在 main() 阶段扫描 10 类 XSS 模式（`script` / `iframe` / `object` / `embed` / `style` / `form` / `svg` / `on*= 事件处理器` / `javascript:` / `data:text/html`）命中即 exit 2 阻断。详见 [`接口契约规范.md`](skills/mcpowers-shared/docs/技术规范/接口契约规范.md) §1（5 字段契约权威定义）+ §1.C.1（v4.0.0+ 业务接口响应规范铁律）+ §1.E（v4.0.1+ API 文档零引用铁律）+ §1.F（v4.2.0+ 表格排版防护 + XSS 阻断铁律）+ [`Swagger字段契约.md`](skills/mcpowers-shared/docs/技术规范/Swagger字段契约.md)（项目自定义必填字段清单机制，`.swagger-required-fields.yml`）。栈级落地：`hooks/pre-write-confirm-api-hint.sh` 由 v2.4.0 软提醒升级为 v2.31.0 真硬门禁（wrapper→helper 集中模式：`scripts/swagger-contract-check.sh` → `scripts/swagger-stack-detect.sh` → `scripts/swagger-required-fields.sh` → `scripts/swagger-lint-helper.py`，不向用户项目注入任何文件）；Flask/Flasgger 实现层参考 [`swagger_template.md`](skills/mcpowers-shared/docs/API文档/swagger_template.md) 24 类接口模板；**`swagger-lint-helper.py check_business_api_responses()` 在 PreToolUse 阶段检测业务接口误列 4xx/5xx**——按 `/`-`_-.` 段内切分避免 `profile` 含 `file` 等子串误判；**`swagger-lint-helper.py check_no_reference_words()` + `export_docs.py check_no_reference_words_spec()` 双层检测零引用字眼**——YAML 字段名行（`key:` 末尾冒号且无 value）跳过不扫；**`export_docs.py _md_cell_safe()` 在导出时统一规整表格单元格**——7 个表格生成点 + 顶层 description 全部集中走该函数,防止 description / example 含换行 / `|` / 不可见字符破坏 Markdown 表格语法；**`export_docs.py _scan_xss_risk()` 在 main() 阶段扫描 XSS / HTML 注入**——10 类危险模式命中即 exit 2。审查门禁：`mcpowers-code-review` 增 R13 反模式条目（swagger 接口 5 字段不完整）+ R14（v4.0.0+ 业务接口 responses 误列 4xx/5xx）+ R15（v4.0.1+ API 文档含禁用引用字眼）+ R16（v4.2.0+ 导出文档含未规整单元格 / XSS 注入风险）+ Quick-Check 段含 3 条扫描命令（grep `tags:` 顶层字段名 / `description:` + `example:` 子字段配对 / `responses:` 块含 ≥ 2 个状态码）+ 1 条 v4.0.0+ 业务接口响应规范扫描命令 + 1 条 v4.0.1+ 零引用字眼扫描命令 + 1 条 v4.2.0+ 表格 `_md_cell_safe()` 覆盖扫描命令（grep `lines.append(f'| {prop_name}` 模式 = 漏走 _md_cell_safe）+ 1 条 v4.2.0+ XSS 阻断扫描命令（grep `<script>` / `javascript:` 模式）。

**接口文档 SSOT 终态收敛（v4.4.0+ 用户决策 D 续·全栈适用铁律）**：用户决策 D 三轮合龙——v4.0.0 业务接口响应规范 + v4.0.1 接口零引用 + v4.3.0 代码/配置零引用 + **v4.4.0 接口 docstring 通用响应/分页/认证字段复用 + description 字段零冗余**。**v4.4.0+ description 字段禁用 8 类内容**（接口契约规范 §1.A.1）：HTTP 状态码（"业务接口 HTTP 永远 200"）/ 认证方式（"需 JWT Bearer Token"）/ 错误码清单（"10001 用户不存在"列表）/ 响应结构（"返回 {code, msg, data}"）/ 完整路径前缀（"完整路径 /api/v1/ec/auth/login"）/ 通用约束（"业务接口统一响应格式"）/ 路径内模块名（"用户管理接口"）+ summary 同义重复——**判别口诀**：删掉这段文字后对接方是否还能直接调通这个接口？能就说明是冗余，删。**v4.4.0+ 通用响应/分页必须用 `$ref` 复用**（接口契约规范 §1.F）——5 个全局组件（`StandardResponse` / `BizResponse` / `PageResponse` / `BizError` / `FileResponse`）+ `BearerAuth` 安全声明 + 全局 `security` 在 `Swagger(app, template={..., **SWAGGER_TEMPLATE})` 一次性注入；接口 docstring `responses.200.schema` / `responses.200.examples` / `security` 全部写 `$ref` 不再展开 `{code, msg, data}` / `{records, page_no, ...}` / `Bearer: []`；**v4.4.0+ 接口路径不重复 basePath / 蓝图前缀**——基础路径在 `Swagger(app, template=...)` 的 `basePath` 字段声明；蓝图前缀在 `Blueprint(url_prefix=...)` 声明；接口路径在 `@bp.route` 装饰器声明——description 字段不再写完整路径。栈级落地：新文件 [`skills/mcpowers-shared/docs/API文档/swagger_components.md`](skills/mcpowers-shared/docs/API文档/swagger_components.md)（5 全局组件 SSOT 权威定义）+ [`skills/mcpowers-shared/docs/API文档/flask_swagger_config.py`](skills/mcpowers-shared/docs/API文档/flask_swagger_config.py)（Flask Flasgger 注入模板常量 `SWAGGER_TEMPLATE`）；[`Flask后端规范.md`](skills/mcpowers-shared/docs/技术规范/Flask后端规范.md) §11.5（应用工厂注入 4 步）+ [`swagger_template.md`](skills/mcpowers-shared/docs/API文档/swagger_template.md) 19 类模板 v4.4.0+ 重写（13 类基础 CRUD + 6 类扩展 + 3 类认证模板统一改用 `$ref`，description ≤ 30 字简短）；`swagger-lint-helper.py` **3 个新检查函数**（v4.4.0+ WARNING 不阻断，v4.5.0 升级为 ERROR）：`check_description_redundant_content()`（8 类禁用内容扫描）/ `check_no_path_in_description()`（完整路径前缀扫描）/ `check_no_repeated_schema()`（`{code, msg, data}` / `{records, page_no, ...}` 内联展开扫描）。审查门禁：`mcpowers-code-review` 增 R18 反模式条目（v4.4.0+ 接口 docstring 冗余内容 + `$ref` 复用铁律违反，description 字段禁用清单 + 通用响应/分页结构必须用 `$ref`）+ Quick-Check 段含 4 条扫描命令（8 类描述禁用字眼扫描 / 完整路径前缀扫描 / `$ref` 复用扫描 / 全局组件 `flask_swagger_config.py` 存在性扫描）。

**接口契约四铁律（v4.5.0+ 用户决策 D 续·全栈适用）**：v4.4.0 SSOT 收敛之后，v4.5.0 把 4 条反模式全部升级为 ERROR 级硬门禁（**不**设 WARNING 过渡期）——**§1.G 路径禁动态参数**（`@bp.route` 模板禁含 `<xxx>` / `<int:xxx>`，资源标识走 query/body；例外 `webhook`/`oauth`/`callback`）+ **§1.H HTTP 方法白名单**（业务接口只允许 GET/POST；列表/详情/字典/导出/下载/流式/进度 → GET；创建/更新/删除（单+批量）/导入/上传/bind/webhook → POST；禁 PUT/PATCH/DELETE/HEAD/OPTIONS）+ **§1.I description 禁鉴权字眼**（description / parameters[].description / responses[].description 不重述 JWT/Bearer/需登录/需鉴权 15 类；鉴权走全局 `securityDefinitions` + `security`）+ **§1.J description 禁错误码清单**（description 不罗列「10001 用户不存在」清单；错误码统一在 `responses.examples` 或 `$ref BizError` 组件维护）。栈级落地：`swagger-lint-helper.py` 4 个新检查函数（`check_no_dynamic_path` AST 解析 + webhook/oauth/callback 白名单 / `check_allowed_methods` vs `_ALLOWED_HTTP_METHODS` 白名单 / `check_no_auth_in_description` 15 类字眼 / `check_no_error_codes_in_description` 6 类正则）。审查门禁：`mcpowers-code-review` 增 R19-R22 反模式条目 + Quick-Check 段含 4 条扫描命令。

**§1.K POST 强制 JSON（v4.5.1+ 全栈适用铁律）**：业务接口 HTTP POST 一律 `Content-Type: application/json`，禁 form-urlencoded / multipart——避免后端 `request.form` / `request.files` 兜底致接口语义混乱。**豁免**（路径段关键字）：`/upload` `/attachment` `/import`（文件必须 multipart）；`/webhook` `/callback` `/notify` `/oauth`（第三方协议约束）。栈级落地：`swagger-lint-helper.py check_post_must_be_json()`（methods= 含 POST → 扫 formData/form-urlencoded/multipart 命中即 ERROR）。审查门禁：`mcpowers-code-review` 增 R23 + Quick-Check 含 1 条扫描命令。
...
**代码/配置零引用铁律·智能二分判定（v4.3.0+ 用户决策 D·全栈适用）**：AI 写任何代码注释（`#` 单行 / `"""` docstring / `//` JS / `--` SQL）/ 配置文件（`.yaml` / `.yml` / `.json` / `.ini` / `.toml`）/ `.sh` 头部注释都需遵守——**22 个禁用字眼**（同 R15/R16 共享清单）+ **v4.3.0 新增 4 个口语化补充**（遵循本项目规范 / 遵循团队规范 / 遵循本仓库规范 / 按团队规范）触发**智能二分判定**：**优先级 1 边界豁免**（YAML 字段名行 / Issue 编号 / `${CLAUDE_PLUGIN_ROOT}` 占位符 / swagger 5 字段值）→ 放行；**优先级 2 外部权威**（RFC/PEP/W3C/WHATWG/WCAG/OWASP/ASVS/CWE/CVE/IEEE/ISO/IEC/ECMA/Unicode/SQL/NIST/FIPS/中国国标/POSIX + 公认作者 + 官方 URL + 行业+规范前缀）→ 放行；**优先级 3 内部规范名**（33 份规范 + 别名，共享常量 `_internal_spec_docs.txt`）→ 拦截（internal_spec）；**优先级 4 项目内代码文件路径**（`utils/xxx.py` 等）→ 拦截（internal_code）；**优先级 5 项目内 .md 文档名**（含 CLAUDE.md/README.md/AGENTS.md，**v4.6.3+ 修订：作为参考型文档改走路径白名单豁免**）→ 拦截（internal_md）；**优先级 6 无前缀画蛇添足**（「按规范/根据规范/遵守规范」无外部权威前缀）→ 拦截（unbounded_spec）；**优先级 7 兜底拦截**（含禁用字眼既不外部也不内部）→ 拦截（fallback）。**路径白名单 6 类**（tests/ / fixtures/ / examples/ / templates/ / docs/历史教训/ / CHANGELOG.md）+ **v4.6.3+ 新增 CLAUDE.md / README.md / AGENTS.md 路径白名单**（参考型文档豁免——v4.0.2 §9.5 场景化决策模型回归）+ **v4.6.3+ 新增 _TERM_EXCEPTIONS 规范术语名豁免**（`代码/配置零引用铁律` / `代码/配置零引用智能二分判定` / `R17 零引用` 等 v4.3.0 引入的术语名本身含禁用字眼，整行命中即放行）+ **docstring 降级为 WARNING**（避免整段 docstring 全 ERROR 阻塞）+ **YAML 字段名行跳过**（结构标记放行）。详见 [`代码规范.md`](skills/mcpowers-shared/docs/技术规范/代码规范.md) §11.3.1（智能二分 + 14 反例 + 6 正例权威定义）。栈级落地：共享检测器 `skills/mcpowers-shared/scripts/check_no_ref_words.py`（v4.3.0 新增，被 3 个调用方共用）；共享常量 3 份 `_forbidden_ref_words.txt` + `_internal_spec_docs.txt` + `_external_authority.txt`（与 R15/R16 共用 `_forbidden_ref_words.txt`，避免漂移）；**v4.6.3+ 拦截入口迁移**（用户决策 D 续）：硬门禁从 `pre-write-check-no-ref-words.sh`（PreToolUse Write|Edit|MultiEdit → exit 2 confirm UI，每写一行就被阻塞）迁移到 `pre-bash-guard.sh` 的 git commit 兜底段（仅在 `git commit` 时扫暂存区所有变更文件 → ERROR 违规累计即 exit 2）；`post-write-check-no-ref-words.sh` 软门禁保留（PostToolUse Write|Edit|MultiEdit → exit 0 stderr 提示开发期温和提醒）。**迁移动机**（用户决策 D 续·两大痛点）：① 22 字眼封闭集 vs AI 同义词池无限——这场对抗 hook 永远输；② 每 Edit 一行就被 confirm UI 阻塞，全自动 / CI / 后台场景下整个流程卡住。**迁移后行为**：开发期不被拦截（AI 可任意写），git commit 提交时一次性兜底扫描（提交是真要交付的时刻，被打断反而合理）。**v4.6.3+ 检测器修订（v4.3.0 决策修补）**：v4.6.3 兜底段首次执行暴露 v4.3.0 设计的 2 类冲突——① 术语名冲突：「代码/配置零引用铁律」术语本身含禁用字眼，无差别扫描破坏术语一致性；② 类型冲突：CLAUDE.md / README.md 是 v4.0.2 §9.5 参考型文档，「CLAUDE.md/README.md 无例外」决策与 §9.5 立场直接冲突。v4.6.3 修订：加 `_TERM_EXCEPTIONS` + `CLAUDE.md / README.md / AGENTS.md` 路径白名单。审查门禁：`mcpowers-code-review` 增 R17 反模式条目（代码/配置含禁用引用字眼，3 优先级 + 智能二分 + 路径白名单）+ Quick-Check 段含 **8 条**扫描命令（22 字眼扫描 + docstring 降级 + 外部权威放行 + 内部规范拦截 + 代码文件拦截 + CLAUDE.md/README.md 拦截 + 无前缀拦截 + 共享常量存在性）。

**v4.5.2+ Edit/MultiEdit 硬门禁全量修复（4 个漏配/盲区一次修完）**：v4.3.0 引入时该 hook 仅注册在 Write matcher；v4.5.1 之前 Edit/MultiEdit 漏配——用户在 `ynas-amadeus-service` 项目实测反馈「硬门禁对文档字眼 + 代码注释字眼相关都不生效，新增和编辑都没有拦截」。v4.5.2 修复：① `hooks/hooks.json` 在 `Edit|MultiEdit` matcher 下注册硬门禁 wrapper（与 Write matcher 对齐）；② 检测器 `main()` 增加 Edit 的 `new_string` / MultiEdit 的 `edits[*].new_string` 输入格式归一化（三种工具 stdin JSON 形状统一抽到 `file_path` + `content`）；③ `tests/plugin-verify.sh §7.7` 新增 T11-T15 共 5 个回归测试（Edit/MultiEdit 输入格式兼容 + hooks.json 注册不漏配的硬守护）。**漏配根因（与下条 swagger 同源）**：v4.3.0 引入时 detector 只读 `content` 字段（根因 2），且 hooks.json 仅 Write matcher 注册（根因 1）——三层防护在 Edit/MultiEdit 场景全失效。

**v4.5.2+ duplicate 函数检测器 MultiEdit 支持**：v2.28.x 起的检测器走 3 档判定（①同文件重名 block ②跨文件单行透传 block ③其他跨文件同名放行），但输入解析只读 `content` / `new_string` 字段——MultiEdit 模式 `extract_function_names` 拿 `new_str=""` 直接 return 0 放行。v4.5.2 复用 no-ref-words 同模式 3 档归一化（`edits[*].new_string` 拼接），MultiEdit 也走 3 档判定。`tests/plugin-verify.sh §7.8` T16 锁定 Edit 模式单行透传 → exit 2（防退化）；T17 锁定 MultiEdit 模式单行透传 → exit 2（v4.5.2 新支持）。

**v4.5.2+ spec-frontmatter 检测器 MultiEdit 行为锁定（无 bug，仅加测试）**：`check_spec_frontmatter.py` 自 v2.27.4 起已支持 MultiEdit（`for edit in tool_input.get("edits", [])` 遍历，第 111-120 行），代码无需改动；§7.8 T18 锁定「MultiEdit 删除 `stability` / `last_breaking_change` 字段仍 exit 2」防未来退化。

**v4.5.2+ Swagger 接口契约 PostToolUse 兜底钩子**（用户报告「api 规范没生效」的根因）：PreToolUse `pre-write-confirm-api-hint.sh` 从 stdin 拿 `file_path` 后从磁盘读文件——Write 模式时新文件未落盘，Edit/MultiEdit 模式时读到旧文件。用户在 Edit 模式新增 `@bp.route` 但没写 docstring → PreToolUse 看到旧文件里没这个函数 → 静默放行。v4.5.2 新增 `hooks/post-write-check-swagger.sh`（PostToolUse Write|Edit|MultiEdit 兜底）：文件已写入后从磁盘读最新内容，跑同一 `swagger-contract-check.sh` helper，命中违规 → exit 2 → Claude Code 把 stderr 喂回 Claude 自动修正（强反馈而非 confirm UI）。`hooks.json` PostToolUse matcher 段从 2 个 hook 增到 3 个。`skills/mcpowers-shared/scripts/swagger-contract-check.sh` 新增 `--content-file=` 可选参数（PostToolUse 当前不传，留作 v4.5.3+ PreToolUse Write 模式内容注入预留）。§7.8 T19/T20/T21 锁定。
